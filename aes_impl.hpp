#pragma once

#include <algorithm>
#include <cstring>
#include <cstdint>

namespace aes_impl
{
    // clang-format off
    constexpr uint8_t sbox[256] = {
        
    };

    constexpr uint8_t inv_sbox[256] = {
        
    };
    // clang-format on

    inline uint8_t gmul(uint8_t a, uint8_t b)
    {
        uint8_t p = 0;
        for (int i = 0; i < 8; ++i)
        {
            if (b & 1)
                p ^= a;
            const bool hi = (a & 0x80) != 0;
            a <<= 1;
            if (hi)
                a ^= 0x1b;
            b >>= 1;
        }
        return p;
    }

    inline void sub_bytes(uint8_t* state)
    {
        for (int i = 0; i < 16; ++i)
            state[i] = sbox[state[i]];
    }

    inline void inv_sub_bytes(uint8_t* state)
    {
        for (int i = 0; i < 16; ++i)
            state[i] = inv_sbox[state[i]];
    }

    // AES state is column-major: byte index = row + 4*col
    inline void shift_rows(uint8_t* s)
    {
        uint8_t t;
        // row 1: shift left 1
        t = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = t;
        // row 2: shift left 2
        t = s[2]; s[2] = s[10]; s[10] = t;
        t = s[6]; s[6] = s[14]; s[14] = t;
        // row 3: shift left 3 (= right 1)
        t = s[15]; s[15] = s[11]; s[11] = s[7]; s[7] = s[3]; s[3] = t;
    }

    inline void inv_shift_rows(uint8_t* s)
    {
        uint8_t t;
        // row 1: shift right 1
        t = s[13]; s[13] = s[9]; s[9] = s[5]; s[5] = s[1]; s[1] = t;
        // row 2: shift right 2
        t = s[2]; s[2] = s[10]; s[10] = t;
        t = s[6]; s[6] = s[14]; s[14] = t;
        // row 3: shift right 3 (= left 1)
        t = s[3]; s[3] = s[7]; s[7] = s[11]; s[11] = s[15]; s[15] = t;
    }

    inline void mix_columns(uint8_t* s)
    {
        for (int c = 0; c < 4; ++c)
        {
            const int i = c * 4;
            const uint8_t a0 = s[i], a1 = s[i + 1], a2 = s[i + 2], a3 = s[i + 3];
            s[i]     = gmul(a0, 2) ^ gmul(a1, 3) ^ a2 ^ a3;
            s[i + 1] = a0 ^ gmul(a1, 2) ^ gmul(a2, 3) ^ a3;
            s[i + 2] = a0 ^ a1 ^ gmul(a2, 2) ^ gmul(a3, 3);
            s[i + 3] = gmul(a0, 3) ^ a1 ^ a2 ^ gmul(a3, 2);
        }
    }

    inline void inv_mix_columns(uint8_t* s)
    {
        for (int c = 0; c < 4; ++c)
        {
            const int i = c * 4;
            const uint8_t a0 = s[i], a1 = s[i + 1], a2 = s[i + 2], a3 = s[i + 3];
            s[i]     = gmul(a0, 14) ^ gmul(a1, 11) ^ gmul(a2, 13) ^ gmul(a3, 9);
            s[i + 1] = gmul(a0, 9)  ^ gmul(a1, 14) ^ gmul(a2, 11) ^ gmul(a3, 13);
            s[i + 2] = gmul(a0, 13) ^ gmul(a1, 9)  ^ gmul(a2, 14) ^ gmul(a3, 11);
            s[i + 3] = gmul(a0, 11) ^ gmul(a1, 13) ^ gmul(a2, 9)  ^ gmul(a3, 14);
        }
    }

    inline void xor_block(uint8_t* dst, const uint8_t* key)
    {
        for (int i = 0; i < 16; ++i)
            dst[i] ^= key[i];
    }

    inline uint32_t sub_word(uint32_t w)
    {
        return static_cast<uint32_t>(sbox[w & 0xff]) | (static_cast<uint32_t>(sbox[(w >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(sbox[(w >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(sbox[(w >> 24) & 0xff]) << 24);
    }

    inline uint32_t rot_word(uint32_t w)
    {
        return (w << 8) | (w >> 24);
    }

    // --- High-level AES-NI equivalents ---

    inline void aesenc(uint8_t* state, const uint8_t* round_key)
    {
        shift_rows(state);
        sub_bytes(state);
        mix_columns(state);
        xor_block(state, round_key);
    }

    inline void aesenclast(uint8_t* state, const uint8_t* round_key)
    {
        shift_rows(state);
        sub_bytes(state);
        xor_block(state, round_key);
    }

    inline void aesdec(uint8_t* state, const uint8_t* round_key)
    {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        inv_mix_columns(state);
        xor_block(state, round_key);
    }

    inline void aesdeclast(uint8_t* state, const uint8_t* round_key)
    {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        xor_block(state, round_key);
    }

    inline void aesimc(uint8_t* dst, const uint8_t* src)
    {
        std::memcpy(dst, src, 16);
        inv_mix_columns(dst);
    }

    inline void aeskeygenassist(uint8_t* dst, const uint8_t* src, uint8_t rcon)
    {
        uint32_t x1, x3;
        std::memcpy(&x1, src + 4, 4);
        std::memcpy(&x3, src + 12, 4);

        const uint32_t sw1 = sub_word(x1);
        const uint32_t sw3 = sub_word(x3);
        const uint32_t rw1 = rot_word(sw1) ^ static_cast<uint32_t>(rcon);
        const uint32_t rw3 = rot_word(sw3) ^ static_cast<uint32_t>(rcon);

        std::memcpy(dst, &sw1, 4);
        std::memcpy(dst + 4, &rw1, 4);
        std::memcpy(dst + 8, &sw3, 4);
        std::memcpy(dst + 12, &rw3, 4);
    }
}
